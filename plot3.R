plot3 <- function() {
    ...
    with (Feb_data, plot(datetime, Global_active_power, 
         type = "n", 
         ylim = c(1, 38),
         ylab = "Energy sub metering",
         xlab = ""))
 
    with(subset(Feb_data, Sub_metering_1 >= 0), 
             points(datetime, Sub_metering_1, type = "l"))
    with(subset(Feb_data, Sub_metering_2 >= 0), 
             points(datetime, Sub_metering_2, type = "l", col = "red"))
    with(subset(Feb_data, Sub_metering_3 >= 0), 
